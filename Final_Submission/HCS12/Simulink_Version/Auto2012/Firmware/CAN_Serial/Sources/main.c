...
volatile UINT8 carPositionUpdated = 0;

volatile UINT8 acc_temp;
...
interrupt 20 void SCIRx_vect(void)   //receive messages from serial port
{
    ...
    switch(serialRxState)   //serialdata (AA | CC | ...)
    {
        ...
        case 1:
            ...
            {
                serialDataLength = sizeof(Params);//+1;
                serialRxChksum ^= 0xCC;
            ...
            else
            {
                if(serialData == serialRxChksum)
                {
                    if(!carParamsUpdated) // Only update when old value has been used
                    {
                        ...
                        //memcpy(distance, serialRxBuffer + 8, sizeof(StabMsg));
                        carParamsUpdated = 1;
...
interrupt 38 void CANRx_vect(void)     //receive messages via CAN bus
{
    ...
    else if(identifier == CAN_SENSOR_MSG_ID) //corrected acceleration messages
    {
        CarSensor carSensor;
        ...
        torcsInput.radarAngle = carSensor.radarAngle;
        torcsInput.speed2 = carSensor.speed2;
        torcsInput.lanePosition = carSensor.lanePosition;
        carInputsUpdated = 1;
    }
...
void main(void)
{
    ...
    for(;;)
    {
        if(carParamsUpdated)   //send simulator's data on CAN bus
        {
            ...
            carDistance.distance = params.dist;
            carDistance.distance1 = params.dist1;
        ...
        if(carPositionUpdated)
        {
            carPosition.X1pos = params.X1pos;
            carPosition.Y1pos = params.Y1pos;
            carPosition.X2pos = params.X2pos;
            carPosition.Y2pos = params.Y2pos;
            //carPosition.speed2 = params.speed2; 
            CANTx(CAN_POSITION_MSG_ID, &carPosition, sizeof(CarPosition));
            
            carPositionUpdated = 0;           
        }
    }
}

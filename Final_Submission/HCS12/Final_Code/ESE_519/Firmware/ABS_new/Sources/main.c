...
UINT8 buffer[12]; // recives the CAN DATA
...
void main(void)
{
    ...
    for(;;)
    {
        if(carParamsUpdated)
        {
            ...
            if(driver_input.brake > 1) 
            {
              
              
              if(driver_input.brake== 75) {
                brake_msg.brakeRR=75;
              brake_msg.brakeFR=75;
              }
               else 
               if (driver_input.brake==25) {
                brake_msg.brakeRL=75;
               brake_msg.brakeFL=75;
               }
                else if(driver_input.brake==100)
                {
